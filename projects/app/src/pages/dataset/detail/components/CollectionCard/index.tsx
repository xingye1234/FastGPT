...
const CollectionCard = () => {
  ...
  const { collections, setCollections, Pagination, total, getData, isGetting, pageNum, pageSize } =
  ...
  //计算是否启用删除按钮
  const delDisabledBtn = useMemo(() => {
    return !collections.some((item) => item.isChecked);
  }, [collections]);
  ...
  const allChecked = useMemo(() => {
    return formatCollections.every((item) => item.isChecked);
  }, [formatCollections]);

  //多选
  const handleCheckboxChange = (e: any) => {
    setCollections((pre: DatasetCollectionsListItemType[]) => {
      return pre.map((item: DatasetCollectionsListItemType) => {
        return {
          ...item,
  ...
  const handleItemChecked = (e: any, id: string) => {
    setCollections((pre: DatasetCollectionsListItemType[]) => {
    ...
    openDeleteConfirm(
      async () => {
        collections.forEach((item) => {
          if (item.isChecked) {
            promises.push(delDatasetCollectionById({ id: item._id }));
          }
        });
  ...
  return (
    <Box py={5} px={[4, 8]} h={'97%'}>
      <Header />
      <MyBox isLoading={isLoading} h={'100%'} py={[2, 4]} bgColor={'white'} borderRadius={'md'}>
                  ...
                  >
                    <Td w={'50px'}>
                      <Checkbox
                        ...
                        onChange={(e) => handleItemChecked(e, collection._id)}
                      ></Checkbox>
                    </Td>
