...
import { Box, Textarea, Button, Flex, useTheme, useDisclosure, Text } from '@chakra-ui/react';
...
const Test = ({ datasetId }: { datasetId: string }) => {
  ...
  return (
    <Box h={'100%'} py={5} px={[4, 8]} boxSizing={'border-box'}>
      <Text mb={4} fontWeight={'bold'} fontSize={'lg'}>搜索测试</Text>
      <Box display={['block', 'flex']} h={'97%'} bgColor={'white'} px={1} py={4} borderRadius={'md'}>
        {/* left  */}
        <Box
          h={['auto', '100%']}
          display={['block', 'flex']}
