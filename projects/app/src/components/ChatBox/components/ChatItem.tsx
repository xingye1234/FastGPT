...
const statusText: Record<string, string> = {
  [ChatStatusEnum.loading]: '搜索相关内容',
  [ChatStatusEnum.running]: '研究总结回答'
};
...
${toolResponse}`}
  ...
  const chatStatusMap = useMemo(() => {
    // console.log('------->', statusBoxData?.name); //loading, running
        ...
        {!!chatStatusMap && statusBoxData && isLastChild && (
          <Flex alignItems={'center'} px={3} py={'1.5px'} borderRadius="md" bg={chatStatusMap.bg}>
            <Box
              ...
              mt={'1px'}
            />
            <Box ml={2} color={'myGray.600'} fontSize={'sm'}>
              {statusText[statusBoxData?.status]
                ? statusText[statusBoxData?.status]
                : '分析你的问题...'}
            </Box>
          </Flex>
